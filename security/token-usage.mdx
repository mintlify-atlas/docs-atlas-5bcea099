---
title: "Using Bearer Tokens to Call Protected Endpoints"
...
|---|---|---|
| `/` (usuarios) | GET | `read` |
| `/{id}` (usuarios) | GET | `read` |
| `/` (usuarios) | POST | `write` |
| `/{id}` (usuarios) | PUT | `write` |
| `/{id}` (usuarios) | DELETE | `write` |
| `/usuarios-por-curso` | GET | `read` |
| `/{id}` (cursos) | GET | Bearer (any valid token) |
| All other `/cursos` routes | — | No token required |
