---
title: "Kubernetes Manifests Reference for All Services"
sidebarTitle: "K8s Manifests"
description: "Reference for every Kubernetes manifest in the repository: what each file deploys, ConfigMap and Secret configuration, resource limits, and RBAC requirements."
---
...
| File | Kind | Purpose |
|---|---|---|
| `mysql-pv.yaml` | PersistentVolume | 2 Gi host-path volume for MySQL data at `/data/mysql` |
| `mysql-pvc.yaml` | PersistentVolumeClaim | Claims the `mysql-pv` volume for the MySQL pod |
| `postgres-pv.yaml` | PersistentVolume | 2 Gi host-path volume for PostgreSQL data at `/data/postgres` |
